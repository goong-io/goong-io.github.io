...
git submodule update --init # initialize goong-js git submodule
```

When pulling in new commits that change the `goong-js` submodule,
run `git submodule update` to update the local copy as well.

## Writing API Documentation

API documentation is written as [JSDoc comments](http://usejsdoc.org/) and processed with [documentationjs](http://documentation.js.org/) **in the source code of [goong-js](https://github.com/goong-io/goong-js)**. This repository fetches the source code through a git submodule.
...
The examples section of the locally run documentation will use the GL JS version located in `../goong-js/dist`,
so make sure to have a working minified build in your local copy of the `goong-js` repo (not the submodule).

## Committing and Publishing Documentation

When a new GL JS release goes out, the release manager will make a PR that updates this repo's `goong-js` submodule to point to the new release. When updating the submodule, you may need to run `npm test -- -u` to update Jest snapshots related to the sidebar navigation.
