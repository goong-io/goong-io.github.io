...
function redirect(location) {
    ...
    if (expressionExceptions.indexOf(location.hash.replace('#', '')) > -1) {
        return `/docs/style-spec/expressions/${location.hash}`;
    }
    ...
    if (location.hash === '#other-filter') {
        return `/docs/style-spec/other/#other-filter`;
    }
    ...
    if (
        ...
        return `/docs/style-spec/layers/${location.hash}`;
    }
