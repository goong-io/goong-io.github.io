...
function redirect(location) {
    ...
    if (expressionExceptions.indexOf(location.hash.replace('#', '')) > -1) {
        return `/style-spec/expressions/${location.hash}`;
    }
    ...
    if (location.hash === '#other-filter') {
        return `/style-spec/other/#other-filter`;
    }
    ...
    if (
        ...
        return `/style-spec/layers/${location.hash}`;
    }
    ...
    hash.splice(0, 1);
    return `/style-spec/${page}/${hash.length ? `#${hash.join('-')}` : ''}`;
}
