...
function redirect(location) {
    ...
    if (expressionExceptions.indexOf(location.hash.replace('#', '')) > -1) {
        return `/goong-js-docs/style-spec/expressions/${location.hash}`;
    }
    ...
    if (location.hash === '#other-filter') {
        return `/goong-js-docs/style-spec/other/#other-filter`;
    }
    ...
    if (
        ...
        return `/goong-js-docs/style-spec/layers/${location.hash}`;
    }
    ...
    hash.splice(0, 1);
    return `/goong-js-docs/style-spec/${page}/${
